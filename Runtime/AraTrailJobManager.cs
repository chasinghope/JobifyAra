...
namespace AraJob
{
    public class AraTrailJobManager : MonoBehaviour
    {
        ...
        public const int POINT_CHUNK_SIZE = 100;
        public const int VERTICES_SIZE = 50;
        ...
        [ContextMenu("计算mesh数据初始化内存占用")]
        private void CalcMeshDataMemoryCost()
        {
            unsafe
            {
                int vector2_unit = sizeof(Vector2);
                int vector3_unit = sizeof(Vector3);
                int vector4_unit = sizeof(Vector4);
                int color_unit = sizeof(Color);
                int int_unit = sizeof(int);

                long size = 0;
                size += this.vertices.Length * vector3_unit;
                //size += this.tangents.Length * vector4_unit;
                size += this.vertColors.Length * color_unit;
                size += this.uvs.Length * vector2_unit;
                size += this.tris.Length * int_unit;
                //size += this.normals.Length * vector3_unit;

                //Debug.Log($"memory cost: {size}");
                //Debug.Log($"({vector2_unit} + {2} * {vector3_unit} + {vector4_unit} + {color_unit} + {int_unit} * TRIANGLE_MUL)  * HEAD_SIZE * POINT_CHUNK_SIZE  * VERTICES_SIZE");
                //Debug.Log($"k: {vector2_unit + 2 * vector3_unit + vector4_unit + color_unit + int_unit * TRIANGLE_MUL}");

                Debug.Log($"memory cost: {size}");
                Debug.Log($"({vector2_unit} + {1} * {vector3_unit} + {color_unit} + {int_unit} * TRIANGLE_MUL)  * HEAD_SIZE * POINT_CHUNK_SIZE  * VERTICES_SIZE");
                Debug.Log($"k: {vector2_unit + 1 * vector3_unit + color_unit + int_unit * TRIANGLE_MUL}");
            }
        }
            ...
            if (rHead.accumTime >= rHead.timeInterval)
            {
                if (rHead.emit)
                {
                    ...
                    if (rHead.len_point <= 1 || Vector3.Distance(position, mPoints[(rHead.index_point + rHead.len_point) - 2].position) >= rHead.minDistance)
                    {
                        ...
                        if (rHead.index_point + rHead.len_point >= mPoints.Length)
                        {
                            Debug.LogError($"Error. 预设的point数量 < 需要的point数量, 请修改POINT_CHUNK_SIZE 使其大于{rHead.index_point + rHead.len_point}");
                            return;
                        }
